...
class Output():
    ...
    #     for [fields, count, pattern, lines] in clusters:
    #         subject = []
    #         output = []
    ...
    def out(self, clusters, files):
        assert len(files) == 1
        with open(files[0], 'r') as f:
            lines = f.read().splitlines()

        map_line_indices = defaultdict(list)
        for i, line in enumerate(lines):
            map_line_indices[line].append(i)

        for [fields, count, pattern, lines] in clusters:
            print(
                ' '.join([
                    ' '.join(str(i) for i in map_line_indices[line])
                    for line in lines
                ])
            )
